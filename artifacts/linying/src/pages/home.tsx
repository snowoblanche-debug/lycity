...
export default function HomePage() {
  ...
  const bannerSubtitle = settings?.siteSubtitle || "點播喜歡的歌曲，一起留下今晚的旋律";
  ...
  return (
    <Layout>
      ...
      >
        <div className="absolute inset-0" style={{ background: "rgba(36,52,71,0.38)", backdropFilter: "blur(1px)" }} />
        ...
        <div className="w-full lg:w-[75%] flex flex-col gap-4">
          ...
          <div className="rounded-xl p-4 flex flex-col md:flex-row gap-3 shadow-sm"
            ...
            <div className="relative flex-1">
              ...
              <Input
                placeholder="搜尋歌名或歌手..."
                className="pl-9 bg-white/60 border-border/60 text-foreground placeholder:text-muted-foreground focus-visible:ring-primary/40"
                value={search}
              ...
              songsData?.songs.map(song => (
                <div
                  ...
                  style={{
                    ...
                    border: "1px solid rgba(112,136,163,0.16)"
                  }}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <h3 className="font-semibold text-base text-foreground truncate leading-snug">
                        {song.youtubeUrl ? (
                          ...
                          >
                            {song.title}
                            <Youtube className="h-3 w-3 opacity-50 flex-shrink-0" />
                          </a>
                        ) : song.title}
                      </h3>
                      {song.isPracticing && (
                        <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-md font-medium"
                          style={{ background: "rgba(234,179,8,0.12)", color: "#a37a00", border: "1px solid rgba(234,179,8,0.30)" }}>
                          🐣 修煉中
                        </span>
                      )}
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-foreground/80 font-medium">{song.artist}</span>
                      <span className="w-0.5 h-0.5 rounded-full bg-muted-foreground/40" />
                      <span className="text-muted-foreground">{song.language}</span>
                      <span className="w-0.5 h-0.5 rounded-full bg-muted-foreground/40" />
                      <span className="text-muted-foreground text-xs flex items-center gap-1">
                        <Mic2 className="w-3 h-3" /> 演唱 {song.playCount} 次
                      </span>
                    </div>
                  </div>
                  ...
                  <Dialog open={requestDialogSongId === song.id} onOpenChange={(open) => !open && setRequestDialogSongId(null)}>
                    ...
                    <DialogContent className="sm:max-w-[400px]">
                      <DialogHeader>
                        <DialogTitle className="text-lg font-semibold">點播：{song.title}</DialogTitle>
                        <p className="text-sm text-muted-foreground mt-0.5">{song.artist}</p>
                      </DialogHeader>
                      <div className="grid gap-4 py-3">
                        <div className="grid gap-1.5">
                          <Label htmlFor="requesterName" className="text-sm font-medium">點歌人名稱 <span className="text-destructive">*</span></Label>
                          <Input
                            id="requesterName"
                            value={requesterName}
                        ...
                        <div className="grid gap-1.5">
                          <Label htmlFor="note" className="text-sm font-medium">留言給 聆櫻 <span className="text-muted-foreground font-normal text-xs">（選填）</span></Label>
                          <Input
                            id="note"
                            value={requestNote}
        ...
        <div className="w-full lg:w-[25%]">
          ...
          >
            <div className="px-4 py-3.5 border-b border-border/50 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-foreground tracking-wide text-sm">等候佇列</h2>
              {queueData?.items && queueData.items.length > 0 && (
                <span className="ml-auto text-xs text-muted-foreground">{queueData.items.length} 首</span>
              )}
            </div>
                  ...
                  {queueData.items.map((item, i) => (
                    <div
                      ...
                      style={item.status === 'playing'
                        ? { background: "rgba(112,136,163,0.14)", border: "1px solid rgba(112,136,163,0.35)" }
                    ...
                    >
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-foreground text-sm truncate leading-snug">{item.song?.title}</div>
                          <div className="text-xs text-muted-foreground truncate mt-0.5">{item.song?.artist}</div>
                          <div className="mt-1.5 text-[11px] text-primary/80 font-medium">
                            {item.requesterName}
                          </div>
                          {item.note && (
                            <div className="mt-1 text-[11px] text-muted-foreground italic border-l-2 border-border pl-1.5 leading-relaxed">
                              {item.note}
                            </div>
                          )}
                        </div>
